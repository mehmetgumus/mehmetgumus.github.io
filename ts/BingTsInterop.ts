...
function AddPushpin(sitePin: BingPin): void {
    ...
    let pushpin = new Microsoft.Maps.Pushpin(new Microsoft.Maps.Location(sitePin.latitude, sitePin.longitude), {
...
function AddPolygon(sitePolygons: bingPolygon[]): void {
    for (var i = 0; i < sitePolygons.length; i++) {
        let sitePolygon = sitePolygons[i];
        let rings = sitePolygon.rings.map(i => new Microsoft.Maps.Location(i.latitude, i.longitude));
        let polygon = new Microsoft.Maps.Polygon(rings, sitePolygon.options);
